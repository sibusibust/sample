...
export const ganttDateRange = (tasks: Task[], viewMode: ViewMode) => {
  let newStartDate: Date = tasks[0].scheduledStart ;
  let newEndDate: Date = tasks[0].scheduledStart ;
  for (const task of tasks) {
    if (task.scheduledStart < newStartDate) {
      newStartDate = task.scheduledStart;
    }
    if (task.scheduledEnd > newEndDate) {
      newEndDate = task.scheduledEnd;
    }
  }
  switch (viewMode) {
    case ViewMode.Month:
